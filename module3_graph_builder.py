...
def build_graph(input_dir: Path, output_dir: Path):
    ...
    scenes = json.load(open(scenes_file)) if scenes_file.exists() else []
    shots = json.load(open(shots_file)) if shots_file.exists() else []
    
    tracklets_data = json.load(open(tracklets_file)) if tracklets_file.exists() else {"tracklets": [], "static_objects": []}
    tracklets = tracklets_data.get("tracklets", [])
    static_objects = tracklets_data.get("static_objects", [])
    
    actions = json.load(open(actions_file)) if actions_file.exists() else []
    
    # Create temporal intersection index
    print("[Module 3] Running temporal intersection matching...")
    ...
    for shot in shots:
        ...
        for track in tracklets:
            ...
            if overlap > 0.5: # 0.5 second threshold
